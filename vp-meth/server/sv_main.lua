...
if TableConfig.Settings['CoreVersion'] == 'newqb' then TriggerEvent(TableConfig.Settings['CoreEvent'], function(obj) Core = obj end) end
        ...
        if TableConfig.Settings['Tables'][Type] ~= nil then
            if Player.Functions.RemoveMoney(TableConfig.Settings['Tables'][Type]['Account'], TableConfig.Settings['Tables'][Type]['Price'], 'Table-Bought') then
                if Player.Functions.AddItem(TableConfig.Settings['Tables'][Type]['ItemName'], 1, 'Table-Bought') then
                    Player.Functions.Notify(TableConfig.Locales['General']['Bought'], 'success')
                end
            else
                Player.Functions.Notify(TableConfig.Locales['Error']['Money'], 'error')
            end
        end
    end
end)
